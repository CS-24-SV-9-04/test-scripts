...
resultList: Dict[str, StrategyResults] = dict()

def createName(filePath, strategy):
    return str(filePath.name) + ":" + strategy

resultFilePaths = list(Path("./many_results/").glob(f"*.tar"))
                        ...
                        with resultTar.extractfile(errInfo) as errFile:
                            ...
                            for result in results:
                                ...
                                if name not in resultList:
                                    resultList[name] = StrategyResults(resultFilePath.name, result.strategy, dict())
                                resultList[name].results[result.query_instance.get_key()] = (result)
                except KeyError:
...
for strategy in resultList.values():
